...
FLOAT_THRESHOLD = 100  # |V| above this => floating island artifact, skip it
...
def net_cluster_refs(layers):
    """Group islands by rounded median voltage (= net) across all layers."""
    clusters = {}
    for pieces in layers.values():
        for P, V, T in pieces:
            if np.abs(V).max() > FLOAT_THRESHOLD:
                continue
            clusters.setdefault(round(float(np.median(V)), 2), []).append(np.median(V))
