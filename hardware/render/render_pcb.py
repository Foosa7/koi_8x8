...
def add_camera(lo: Vector, hi: Vector, view: str, margin: float) -> bpy.types.Object:
    ...
    if view in ("top", "bottom"):
        cam_data.type = "ORTHO"
        cam_data.ortho_scale = max(hi.x - lo.x, hi.y - lo.y) * margin
        ...
        cam.location = center + direction * (radius / math.sin(half_fov)) * margin
...
def main() -> None:
    ...
    add_camera(lo, hi, args.view, args.margin)
